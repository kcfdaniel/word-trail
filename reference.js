...
dsLoad({
  ...
  onLoad(ctx) {
    ...
    const stop = ctx.effect(() => {
      try { el.classList.toggle('hidden', !fn()) }
      catch (_) { }
    })
    return () => stop?.()
  },
...
class TeleprompterApp {
  ...
  registerEventListeners() {
    ...
    if (recordBtn) recordBtn.addEventListener('click', () => {
      ...
      this.saveSettings()
      try { recordBtn.setAttribute('aria-pressed', this.state.recordMe ? 'true' : 'false') }
      catch (_) { }
  ...
  loadInitialState() {
    ...
    if (this.isMobileUA) {
      try { this.dom.scrollModeSelect.querySelector('option[value="voice"]').disabled = true }
      catch (_) { }
  ...
  enableAds() {
    try { document.body.classList.remove('ads-off') }
    catch (_) { }
  ...
  toggleMirrorText(isMirrored) {
    ...
    try { if (this.dom.videoPreview) this.dom.videoPreview.style.transform = this.state.isMirrored ? 'scaleX(-1)' : 'scaleX(1)' }
    catch (_) { }
    DS.mergePatch({ settings: { isMirrored: this.state.isMirrored } })
    try { if (this.dom.mirrorToggleBtn) this.dom.mirrorToggleBtn.setAttribute('aria-pressed', this.state.isMirrored ? 'true' : 'false') }
    catch (_) { }
  }
  ...
  showRecordingsSidebar(show) { /* no-op: recordings now shown in modal */
    try { this.dom.showRecordingsBtn.setAttribute('aria-expanded', show ? 'true' : 'false') }
    catch (_) { }
  }
  ...
  showModal(type) {
    if (!type) {
      ...
      try { this.dom.showRecordingsBtn.setAttribute('aria-expanded', 'false') }
      catch (_) { }
    ...
    if (type === 'videos') {
      try { this.dom.showRecordingsBtn.setAttribute('aria-expanded', 'true') }
      catch (_) { }
    }
  ...
  updateScrollSpeed(speed) {
    ...
    if (input) {
      try { input.value = String(this.state.scrollSpeed) }
      catch (_) { }
    }
  ...
  startAutoScroll() {
    ...
    try { this.dom.teleprompterDisplay.style.scrollBehavior = 'auto' }
    catch (_) { }
  ...
  stopAutoScroll() {
    ...
    try { this.dom.teleprompterDisplay.style.scrollBehavior = 'smooth' }
    catch (_) { }
  }
    ...
    try {
      ...
      if (!recordMe) {
        ...
        if (wantVoice) {
          ...
          try { this.dom.teleprompterDisplay.scrollTo({ top: 0, behavior: 'auto' }) }
          catch (_) { }
          this.startAutoScroll()
      ...
      if (!this.state.currentHasVideo) try { this.dom.videoPreviewContainer.classList.add('hidden') }
      catch (_) { }
  ...
  setupMediaRecorder() {
    ...
    this.state.mediaRecorder.onstop = async () => {
      ...
      let thumb = null
      try { thumb = await this.captureThumbnail(blob) }
      catch (_) { thumb = null }
    ...
    if (count === 0) {
      ...
      this.state.sessionRecordings.forEach((rec) => {
        ...
        player.addEventListener('loadedmetadata', () => {
          ...
          if (vw && vh) {
            try { player.style.aspectRatio = `${vw} / ${vh}` }
            catch (_) { }
          }
    ...
    if (SpeechRecognition) {
      ...
      this.recognition.onend = () => {
        ...
        this.recRestartTimer = setTimeout(() => {
          try { this.recognition.start(); DS.mergePatch({ ui: { recStatus: 'restarting' } }); this.log('SR restart') }
          catch (e) { this.log('SR restart error', e) }
        }, 350)
      }
      this.recognition.onerror = (e) => {
        ...
        clearTimeout(this.recRestartTimer)
        this.recRestartTimer = setTimeout(() => {
          try { this.recognition.start(); DS.mergePatch({ ui: { recStatus: 'recovering' } }); this.log('SR recover start') }
          catch (err) { this.log('SR recover error', err) }
        }, 500)
      }
    }
    else {
  ...
  startSpeechRecognition() {
    ...
    catch (e) { this.log('SR start error', e); DS.mergePatch({ ui: { recStatus: 'error', recError: String(e?.message || e) } }) }
  }
  ...
  updateUIMode() {
    ...
    try { if (this.dom.micLevel) this.dom.micLevel.classList.toggle('hidden', !isRecording || !this.state.recordMe) }
    catch (_) { }
  }

  // --- Utilities ---
  log(...args) {
    try { if (this.debug) console.log('[VS]', ...args) }
    catch (_) { }
  }
  ...
  stopMicMeter() {
    try { if (this.micRAF) cancelAnimationFrame(this.micRAF); this.micRAF = 0 }
    catch (_) { }
    try { this.analyser = null; this.micData = null }
    catch (_) { }
    try { this.audioCtx?.close?.(); this.audioCtx = null }
    catch (_) { }
    try { if (this.dom.micFill) this.dom.micFill.style.width = '0%' }
    catch (_) { }
  }
...
TeleprompterApp.prototype.switchToVideoOnlyForVoice = async function () {
  try {
    ...
    try { if (this.state.mediaRecorder && this.state.mediaRecorder.state !== 'inactive') this.state.mediaRecorder.stop() }
    catch (_) { }
    try { this.state.stream?.getAudioTracks?.().forEach(t => t.stop()) }
    catch (_) { }
